...
source "amazon-ebs" "portal" {
  access_key = "${var.access_key}"
  secret_key = "${var.secret_key}"
  region     = "${var.region}"

  ami_name                    = "SBCPP-PORTAL-${local.timestamp}"
  associate_public_ip_address = true
  instance_type               = "t3.medium"
  source_ami                  = "${data.amazon-ami.portal.id}"
  user_data_file              = "./ami/packer/portal/scripts/UserData.ps1"
...
build {
  sources = ["source.amazon-ebs.portal"]
  ...
  provisioner "file" {
    ...
    source      = "./ami/packer/portal/scripts/"
  }
